#!/usr/bin/env python3
"""
Download the exact 109 X-media images referenced by the supplied Curaçao thread
and convert them into the local WebP files expected by curacao.json.

Run from the repository root:

    pip install requests pillow
    python download-curacao-images.py
"""
...
manifest = json.loads(
    (ROOT / "curacao-image-manifest.json").read_text(encoding="utf-8")
)
...
print(f"\nDone. {len(manifest)} Curaçao source images are now local WebP assets.")
