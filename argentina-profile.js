(() => {
  ...
  const renderHeroLogo = (hero) => {
    if (!hero.logo && !hero.fallbackLogo) return "";
  ...
  const renderHero = ({ hero, meta }) => `
    <section class="profile-hero" aria-labelledby="country-title">
      ...
      <div class="profile-source-strip">
        <span>Source: ${esc(meta.source.label)}</span>
        <span>${esc(meta.source.file)}</span>
      </div>
    </section>
  `;
  ...
  const renderNav = (sections) => `
    <nav class="profile-nav" aria-label="Argentina profile sections">
      ${sections
        .map(
          (section) => `
            ...
            lineup.image
              ? `
                <figure class="profile-lineup-image profile-media">
                  ${renderImage(
                    lineup.image,
                    "Argentina historical line-up graphic"
                  )}
                </figure>
              `
              : ""
                      ...
                      player.image
                        ? `
                          <figure class="profile-player-photo">
                            ${renderImage(
                              player.image,
                              `${player.name} — Argentina historical line-up`,
  ...
  const renderHost = (section) => `
    <div class="profile-host-card">
      <div>
        <p class="profile-host-kicker">
          ${esc(section.host.kicker || "Qualified host")}
        </p>
        ...
        section.host.image
          ? `
            <figure class="profile-host-image profile-media">
              ${renderImage(
                section.host.image,
                "Argentina 2030 host qualification graphic"
              )}
            </figure>
          `
          : ""
  ...
  const renderSection = (section) => `
    ...
    >
      <header class="profile-section-header">
        ...
        <div>
          <p class="profile-section-kicker">The 211 Files · Argentina</p>
          ...
          ${
            section.intro
              ? `<p class="profile-section-intro">${esc(section.intro)}</p>`
              : ""
          }
        </div>
      </header>
    ...
    app.innerHTML = `
      ...
      <div class="profile-shell">
        ...
        <footer class="profile-footer">
          <p>
            Page content is structured from the attached Argentina source document.
            The source’s chronology, framing and substantive claims have been retained,
            with light grammar and spacing normalisation for web presentation.
          </p>
        </footer>
      </div>
    `;
    ...
    .catch((error) => {
      ...
      app.innerHTML = `
        <div class="profile-error">
          <h1>The Argentina page could not be loaded</h1>
          <p>
