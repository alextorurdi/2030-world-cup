(() => {
  ...
  const renderImage = (image, alt, className = "profile-media-image") => `
    <img class="${className}" src="${esc(image)}" alt="${esc(alt)}"
  ...
  const renderNav = (sections) => `
    <nav class="profile-nav" aria-label="Morocco profile sections">
      ${sections.map((section) => `
        <a href="#${esc(section.id)}">
          <span>${esc(section.number)}</span>${esc(section.title)}
        </a>
      `).join("")}
    </nav>
  `;
  ...
  const renderEntry = (entry, sectionTitle) => `
    <article class="profile-story-entry">
      <div class="profile-story-copy">
        ...
        ${(entry.paragraphs || []).map((p) => `<p>${esc(p)}</p>`).join("")}
      </div>
        ...
        <div>
          ${lineup.image ? `
            <figure class="profile-lineup-image profile-media">
              ${renderImage(lineup.image, "Morocco historical line-up graphic")}
            </figure>
          ` : ""}
              ...
              <article class="profile-player-card">
                ${player.image ? `
                  <figure class="profile-player-photo">
                    ${renderImage(
                      player.image,
                      `${player.name} — Morocco historical line-up`,
    ...
    <div class="profile-host-card">
      ...
      ${section.host.image ? `
        <figure class="profile-host-image profile-media">
          ${renderImage(section.host.image, "Morocco 2030 host graphic")}
        </figure>
      ` : ""}
    </div>
  `;
  ...
  const renderSection = (section) => `
    <section id="${esc(section.id)}" class="profile-section"
      aria-labelledby="${esc(section.id)}-title">
      <header class="profile-section-header">
        <span class="profile-section-number">${esc(section.number)}</span>
        <div>
          <p class="profile-section-kicker">The 211 Files · Morocco</p>
      ...
      <div class="profile-section-body">${renderSectionContent(section)}</div>
    </section>
  `;
    ...
    app.innerHTML = `
      ...
      <div class="profile-shell">
        ...
        <footer class="profile-footer">
          <p>
            Content structured from the supplied Morocco thread export.
            Project-specific 2025–26 and 2030 claims are preserved as source material.
          </p>
          <a href="${esc(data.meta.source.url)}" target="_blank" rel="noopener noreferrer">
  ...
  fetch(dataPath, { cache: "no-store" })
    .then((response) => {
      if (!response.ok) throw new Error(`Could not load ${dataPath}: ${response.status}`);
    ...
    .catch((error) => {
      ...
      app.innerHTML = `
        <div class="profile-error">
          <h1>The Morocco page could not be loaded</h1>
          <p>Check that <code>${esc(dataPath)}</code> exists and contains valid JSON.</p>
        </div>
      `;
    });
})();
