(() => {
  ...
  const renderNav = (sections) => `
    <nav class="profile-nav" aria-label="Spain profile sections">
      ${sections
        .map(
          (section) => `
  ...
  const renderEntry = (entry, sectionTitle) => `
    <article class="profile-story-entry">
      <div class="profile-story-copy">
        ${
          entry.date
            ? `<p class="profile-entry-date">${esc(entry.date)}</p>`
            : ""
        }
            ...
            lineup.image
              ? `
                <figure class="profile-lineup-image profile-media">
                  ${renderImage(lineup.image, "Spain historical line-up graphic")}
                </figure>
              `
              : ""
          }
  ...
  const renderHost = (section) => `
    <div class="profile-host-card">
      <div>
        <p class="profile-host-kicker">Qualified host</p>
        ...
        section.host.image
          ? `
            <figure class="profile-host-image profile-media">
              ${renderImage(section.host.image, "Host qualification graphic")}
            </figure>
          `
          : ""
      }
  ...
  const renderSection = (section) => `
    ...
    >
      <header class="profile-section-header">
        ...
        <div>
          <p class="profile-section-kicker">The 211 Files · Spain</p>
    ...
    app.innerHTML = `
      ...
      <div class="profile-shell">
        ...
        <footer class="profile-footer">
          <p>
            Page content structured from the supplied Thread Reader PDF. Source order,
            section numbering and substantive claims follow that document.
          </p>
    ...
    .catch((error) => {
      ...
      app.innerHTML = `
        <div class="profile-error">
          <h1>The Spain page could not be loaded</h1>
          <p>
