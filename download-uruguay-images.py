#!/usr/bin/env python3
"""
Download the exact 147 X-media images referenced by the supplied Uruguay thread
and convert them into the local WebP files expected by uruguay.json.

Run from the repository root:

    pip install requests pillow
    python download-uruguay-images.py
"""
...
manifest = json.loads(
    (ROOT / "uruguay-image-manifest.json").read_text(encoding="utf-8")
)
...
print(f"\nDone. {len(manifest)} Uruguay source images are now local WebP assets.")
